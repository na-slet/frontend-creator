...
export default class EditEventPopup extends React.Component<Props, State> {
	async getUnions() {
		return await axios.get('https://backend.naslet.ru/unions',{params:{}});
	}
	async getEventLogos(){
		return await axios.get('https://backend.naslet.ru/colors?stage=NOT_PARTICIPATED', {params:{}});
	}
	editEvent(){
		console.log(this.state);
		axios.put('https://creator.backend.naslet.ru/user/event', {
			...
			reg_end_date: this.state.reg_end_date === null ? undefined : this.state.reg_end_date,
			start_date: this.state.start_date === null ? undefined : this.state.start_date,
			end_date: this.state.end_date === null ? undefined : this.state.end_date,
	...
	async getEvent(access_token: string, event_id: string){
		return await axios.get('https://creator.backend.naslet.ru/user/events',{params:{
	...
	async componentDidMount() {
		const union_response = await this.getUnions();
		this.setState({unions: union_response.data});
		const logos_response = await this.getEventLogos();
		this.setState({event_logos: logos_response.data})
		const event_response = await this.getEvent(this.props.access_token, this.props.event_id);
		this.setState({
			name: event_response.data[0].name,
			description: event_response.data[0].description,
			short_description: event_response.data[0].short_description,
			visibility: event_response.data[0].visibility,
			price: event_response.data[0].price,
			logo_variant: event_response.data[0].logo_variant,
			city: event_response.data[0].city,
			reg_end_date: event_response.data[0].reg_end_date,
			start_date: event_response.data[0].start_end_date,
			end_date: event_response.data[0].end_date,
			event_type: event_response.data[0].event_type,
			event_category: event_response.data[0].event_category,
			union: event_response.data[0].union,
			min_age: event_response.data[0].min_age,
			max_age: event_response.data[0].max_age,
			address: event_response.data[0].address,
			latitude: event_response.data[0].latitude,
			longitude: event_response.data[0].longitude,
		})

	}

	render(){
		// if (this.props.event_id) this.getEvent(this.props.access_token, this.props.event_id);
		return (
			<Modal
				sx = {{
					top: 0,
				...
				open={this.props.event_id != null}
				onClose={()=>{this.props.setOpen(null);console.log('closing')}}
			...
			>
				<Paper sx={{padding: 5, maxWidth: 500, maxHeight: 500, overflow: 'auto'}} >
					<Container sx={{width: '100%'}}>
						<Stack spacing={2}>
							<Typography>Изменить ивент</Typography>
							<TextField
								required
							...
							<LocalizationProvider dateAdapter={AdapterDayjs}>
								<DateTimePicker
									...
									onChange={(e)=>{this.setState({reg_end_date: e.format()})}}
									renderInput={(params) => <TextField {...params} />}
								/>
								<DateTimePicker
									label="Начало ивента"
									value={this.state.start_date}
									onChange={(e)=>{this.setState({start_date: e.format()})}}
									renderInput={(params) => <TextField {...params} />}
								/>
								<DateTimePicker
									label="Окончание ивента"
									value={this.state.end_date}
									onChange={(e)=>{this.setState({end_date: e.format()})}}
									renderInput={(params) => <TextField {...params} />}
								/>
							...
							/>
							<Button variant="contained" onClick={()=>{this.editEvent()}}>Создать</Button>
						</Stack>
					</Container>
				</Paper>
			</Modal>
